...
All notable changes to `voice-satellite-esp32` are documented in this file.

## 1.0.0 - 2026-08-30

First stable release of the standalone ESP32 / ESP32-S3 Voice Satellite satellite firmware.

### Added

#### Voice and audio

- Added authenticated `voice.satellite.v1` WebSocket communication with Bearer token support.
- Added microphone uplink using PCM16 / 16 kHz / mono WAV audio.
- Added full STT → Assistant → TTS voice roundtrips.
- Added `VOICE_SATELLITE_AUTO_TTS` for normal voice interactions.
...
- Added configurable TTS quality selection through `VOICE_SATELLITE_TTS_QUALITY`.
...
- Added a full Waveshare touch UI inspired by the Voice Satellite Command Center.
...
- Added configurable display rotation through `VOICE_SATELLITE_DISPLAY_ROTATION`.
- Supported rotations:
  - `0`
  - `1` / 90°
  - `2` / 180°
...
- Added configurable maximum recording duration through `VOICE_SATELLITE_RECORD_MS`.
...
- Added local conversion to Voice Satellite PCM16 / 16 kHz / mono.
...
- Adapted the ESP32 Voice uplink to the production `voice.satellite.v1` contract already used by the Linux / ReSpeaker satellites:
  - `session.start`
  - binary WAV message
  - `audio.commit`
...
- WakeNet and the normal Voice Satellite recording path share the microphone safely by pausing WakeNet before STT recording.
...
- Created the standalone `voice-satellite-esp32` repository.
...
- Added `voice.satellite.v1` WebSocket protocol support.
