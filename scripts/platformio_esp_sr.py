# Voice Satellite ESP32-S3 WakeNet model integration for pioarduino.
#
...
# binary as an extra flash image at the Voice Satellite model partition offset.
...
if PIOENV == "waveshare-1_85c":
    ...
    if local_config.is_file():
        ...
        match = re.search(
            r"^\s*#\s*define\s+VOICE_SATELLITE_WAKEWORD_ENABLED\s+([^\s/]+)",
            text,
            re.MULTILINE,
        )
        if match:
    ...
    if enabled:
        ...
        if not libs_dir:
            raise RuntimeError("Voice Satellite ESP-SR: framework-arduinoespressif32-libs nicht gefunden")

        model = Path(libs_dir) / "esp32s3" / "esp_sr" / "srmodels.bin"
        if not model.is_file():
            raise RuntimeError(
                "Voice Satellite ESP-SR: srmodels.bin fehlt: %s" % model
            )
        ...
        if model_size > max_model_bytes:
            raise RuntimeError(
                "Voice Satellite ESP-SR: Modell ist zu gross (%d > %d Bytes)"
        ...
        print(
            "Voice Satellite ESP-SR: Wakeword aktiv; srmodels.bin (%d Bytes) wird bei 0xC10000 mitgeflasht."
            % model_size
        )
    else:
        print("Voice Satellite ESP-SR: Wakeword aus; Modell-Upload wird uebersprungen.")
