...
callback. Voice Satellite TTS PCM/WAV responses can be several hundred KiB, while the
Waveshare has 8 MiB PSRAM. This build-time patch raises the frame limit and
places large receive buffers in PSRAM.

The repair section also fixes files modified by the first Voice Satellite version of
...
marker = "VOICE_SATELLITE_WEBSOCKETS_PSRAM_PATCH"

if not header.exists() or not source.exists():
    raise RuntimeError(f"Voice Satellite WebSockets dependency not found under {libdeps}")
...
bad_header_block = (
    f"// {marker}: Voice Satellite TTS can exceed the upstream 15 KiB receive limit.\\n"
...
if "\\n#define WEBSOCKETS_MAX_DATA_SIZE" in h:
    raise RuntimeError("WebSockets.h still contains an escaped newline from an old Voice Satellite patch")
if "#define WEBSOCKETS_MAX_DATA_SIZE (2 * 1024 * 1024)" not in h:
    raise RuntimeError("Voice Satellite WebSockets receive-size define is missing after patch")
...
for escaped in (
    ...
    if escaped in c:
        raise RuntimeError(
            "WebSockets.cpp still contains escaped newlines from an old Voice Satellite patch; "
            "delete .pio/libdeps for this environment and rebuild"
        )

source.write_text(c, encoding="utf-8")
print(f"Voice Satellite WebSockets patch: large inbound frames use PSRAM ({pioenv})")
