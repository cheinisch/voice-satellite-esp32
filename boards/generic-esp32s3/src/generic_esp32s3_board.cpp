...
bool GenericEsp32S3Board::begin() {
    pinMode(VOICE_SATELLITE_GENERIC_BUTTON_PIN, INPUT_PULLUP);
...
void GenericEsp32S3Board::loop() {
    const bool pressed = digitalRead(VOICE_SATELLITE_GENERIC_BUTTON_PIN) == LOW;
...
void GenericEsp32S3Board::showAssistant(const String& text) { Serial.printf("[Display:N/A] Voice Satellite: %s\n", text.c_str()); }
